...
void analyzeMoveList(MoveList *list, Board *gameBoard)
{
  ...
  for (int i = 0; i < list->length; ++i)
  {
    ...
    {
      moveValue += 150; // add 300 to the movevalue if the move will put the opponent in check
      if (movePutsPieceInDanger(thisPiece, destinationSquare, gameBoard) == 1)
      {
...
double getMaxMoveValue(MoveList *list)
{
  MoveListNode *current = list->head, *next;
  double maxValue = 0.0;
  ...
  {
    next = current->nextNode;
    if (current->moveValue > maxValue)
    {
      maxValue = current->moveValue;
    }
    current = next;
  }
...
MoveListNode *chooseRandomNode(MoveList *list)
{
  time_t t;
  ...
  if (list->length != 1)
  {
    int selection = rand() % list->length;

    MoveListNode *current = list->head;
    MoveListNode *next = NULL;
...
int countOccurances(MoveList *list, double moveValue)
{
  MoveListNode *current = list->head, *next = NULL;
  int occurances = 0;

  for (int i = 0; i < list->length && current; ++i) //count number of maxMoves
  {
    next = current->nextNode;
    if (current->moveValue == moveValue)
    {
      occurances++;
    }
    current = next;
