...
//if input message is more than 28 characters it splits it into multiple lines
  const gchar *constmessage = message;
  if (strlen(message) > 28)
  {
    ...
    while(word != NULL)
    {
      ...
      {
        counter += strlen(word);
        const gchar *constword = word;
        gtk_text_buffer_insert_at_cursor (chathistory, constword, len);
	gtk_text_buffer_insert_at_cursor (chathistory, space, len);
	counter += 1;
        word = strtok(NULL, " ");
      }
      else if(counter >28)
      {
	gtk_text_buffer_insert_at_cursor (chathistory, newline, len);
	gtk_text_buffer_insert_at_cursor (chathistory, tab, len);
	counter = 0;
      }
...
//takes the input message from server and prints it to chat
//char inmessage = MESSAGEFROMSERVER;
  char inmessage[100] = "message: Hey good luck man hope you have a good game today";
...
//if message is longer than 21 characters it is split into multiple lines
  if (strlen(trimmed) > 21)
  {
    char *word = strtok(trimmed, " ");
    int counter = 0;
    const char *space = " ";
    const char *tab = "\t";
