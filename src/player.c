...
int movePutsPieceInDanger(ChessPiece *piece, Square *endSquare, Board *gameBoard)
{
  ...
  if (capFlag == 2)
  {
    ...
    startSquare->thisSquaresPiece = NULL;
    endSquare->thisSquaresPiece = piece;
    piece->square = endSquare;
  }
...
MoveListNode *AIHardMove(Player *player, Board *gameBoard)
{
  if (player->type == COMPUTER)
  {
    ...
    if (playerChecked == 1) //if the player is in check
    {
      MoveListNode *bestMove = getBestCheckedMove(possibleMoves, gameBoard);
      deleteMoveList(possibleMoves);
      return bestMove;
    }

    double maxMoveValue = getMaxMoveValue(possibleMoves);

    if (maxMoveValue == 0.0) //if all the moves have the value of 0 (ex. at start of the game). pick a random move
    {
      MoveListNode *retNode = chooseRandomNode(possibleMoves);
      return retNode;
    }

    int numMaxValMoves = countOccurances(possibleMoves, maxMoveValue); //keep track of the number of max value moves
    ...
    else //if there are multiple best moves, choose a random one
    {
      ...
      MoveListNode *selection = chooseRandomNode(bestMoves);
...
MoveListNode *getBestCheckedMove(MoveList *possibleMoves, Board *gameBoard)
{
  MoveList *possibleCheckedMoves = createMoveList();
  ...
  while (current)
  {
    ...
    if (movePutsKingOutOfCheck(current->startSquare, current->endSquare, gameBoard) == 1)
    {
      appendMove(current->startSquare, current->endSquare, possibleCheckedMoves, current->moveValue); //add to p
    }
    current = next;
  }

  double maxCheckedMoveValue = getMaxMoveValue(possibleCheckedMoves);
  time_t t;
  srand((unsigned)time(&t));

  if (maxCheckedMoveValue == 0.0) //if all the moves are the same, pick a random move
  {
    MoveListNode *retNode = chooseRandomNode(possibleCheckedMoves);
    return retNode;
  }

  int numMaxValMoves = countOccurances(possibleCheckedMoves, maxCheckedMoveValue);

  if (numMaxValMoves == 1) // if there is one best move
  {
    current = possibleCheckedMoves->head;
    next = NULL;
    for (int i = 0; i < possibleCheckedMoves->length && current; ++i)
    {
      next = current->nextNode;
      if (current->moveValue == maxCheckedMoveValue)
      {
        return current;
      }
      current = next;
  ...
  else //if there are multiple best moves, choose a random one
  {
    ...
    for (int i = 0; i < possibleCheckedMoves->length && current; ++i) // add all the best moves to the move list if there are multiple best moves
    {
      next = current->nextNode;
    ...
    deleteMoveList(possibleCheckedMoves);
